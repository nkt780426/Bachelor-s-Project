...
# in_planes: số lượng channel vào của tensor. Gọi plane là vì mỗi channel là 1 mặt phẳng 2D.
# out_planes: số lượng channel ra của tensor
# Hàm trả về Conv 3x3
...
# Là khối cơ bản trong iResnet thực hiện x[l+1] = x[l] + F(x[l])
class Bottleneck(nn.Module):
    # Là thuộc tính được định nghĩa để mở rộng số lượng kênh đầu ra khi kết hợp các khối với nhau.
    expansion = 4

    # downsample: thay đổi kích thước số lượng kênh cho phù hợp để cộng residual.
    # norm_layer: là lớp BN ở cuối.
    # exclude_bn0: phục vụ cho Block đầu ở Middle ResBlock không có BN
...
class iResNet(nn.Module):
    ...
    # zero_init_residual: Nếu là True, các trọng số của BatchNorm trong các residual branch (nhánh còn lại sau khi cộng với đầu vào) sẽ được khởi tạo bằng 0.
...
def iresnet101(pretrained=False, **kwargs):
    ...
    model.load_state_dict(torch.load('/home/vohoang/.cache/torch/pretrained/iresnet101.pth', weights_only=True))
    return model